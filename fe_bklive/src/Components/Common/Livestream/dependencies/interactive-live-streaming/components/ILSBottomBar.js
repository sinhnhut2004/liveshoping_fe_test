...
import { useLivestreamContext } from 'App.js';
import VoucherOverlay from 'UI/elements/VoucherOverLay';
...
export function ILSBottomBar({
  ...
  const { showPinedProduct, isShowPinedProduct } = useLivestreamContext();
  ...
  const GetVoucherBTN = () => {
    ...
    return (
      <button
        class="bg-yellow-500 text-white p-2 rounded text-sm w-auto mr-2"
        onClick={openVoucherOverlay}>
        Voucher
      </button>
    );
  };
  const BuyNowBtn = ({ handleClick }) => {
    // const {} = useMeeting();

    return (
      <button
        class="bg-red-600 text-white p-2 rounded text-sm w-auto mr-2"
        onClick={() => {
          if (!isShowPinedProduct) handleClick();
        }}>
        Buy now
      </button>
    );
  ...
  const FollowBTN = () => {
    ...
    return (
      <button className="bg-blue-500 text-white p-2 rounded text-sm w-auto mr-2">
        Follow
      </button>
    );
  };
  ...
  const ShareBTN = () => {
    ...
    return (
      <PopoverAntd content={<SharePopover />} title="Share" trigger="click">
        <button className="bg-blue-600 text-white p-2 rounded text-sm w-auto mr-2 hide-in-fullscreen">
          Share
        </button>
      </PopoverAntd>
    );
  ...
  const [isOpenVoucherOverLay, setOpenVoucherOverlay] = useState(false);

  const closeVoucherOverlay = () => {
    setOpenVoucherOverlay(false);
  };

  const openVoucherOverlay = () => {
    console.log('asdasd');
    setOpenVoucherOverlay(true);
  };
  return isMobile || isTab ? (
    <div
    ...
    <div className="md:flex lg:px-2 xl:px-6 pb-2 px-2 hidden relative">
      <VoucherOverlay
        isOpen={isOpenVoucherOverLay}
        onClose={closeVoucherOverlay}
      />
      {meetingMode === Constants.modes.CONFERENCE && <MeetingIdCopyBTN />}
      <div className="flex items-center justify-center">
        {meetingMode === Constants.modes.VIEWER && <ViewBTN />}
        {meetingMode === Constants.modes.VIEWER && (
          <p className="text-red-500 mr-2">SALE 50% OFF TODAY!!!</p>
        )}
        {meetingMode === Constants.modes.VIEWER && (
          <Link to="/store" className="text-blue-500 mr-2">
            Dadies fashion shop
          </Link>
        )}
      ...
      <div className="flex flex-1 items-center justify-center" ref={tollTipEl}>
        ...
        {meetingMode === Constants.modes.VIEWER && (
          <BuyNowBtn handleClick={() => showPinedProduct(true)} />
        )}
